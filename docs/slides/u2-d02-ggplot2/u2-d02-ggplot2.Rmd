...
.pull-left[
```{r named-args, eval = FALSE, warning=FALSE}
ggplot(data = penguins,
       mapping = aes(x = bill_depth_mm,
                     y = bill_length_mm,
                     colour = species)) +
...
.pull-right[
```{r not-named-args, eval = FALSE, warning=FALSE}
ggplot(penguins,
       aes(x = bill_depth_mm,
           y = bill_length_mm,
           colour = species)) +
...
We won't discuss the following slides, but they serve as a useful resource if you need more faceting options.

---

## There are several ways to facet
...
.pull-left-narrow[
```{r facet-color-legend, fig.show = "hide", warning = FALSE, error=FALSE}
ggplot(
  penguins, 
  aes(x = bill_depth_mm, 
      y = bill_length_mm, 
